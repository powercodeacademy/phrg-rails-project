...
10. Your application should conform to Nitro's Ruby linting conventions. The `rubocop.yml` included in this repo should be copied over to your application, named `.rubocop.yml`, and then running `rubocop` from your application's root should return `no offenses detected` message. (Notice you change the filename by adding a `.` to it.)
